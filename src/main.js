...
renderContainer(desktop, desktopInner)
...
const openFolder = () => {
  ...
  //openFolder.querySelector('.openFolder-inner').
}

function renderContainer(container, containerInner) {
  //нужно замкнуть счетчик папок
  containerInner.map(item => {
    container.insertAdjacentHTML('afterbegin', `${fileTypes[item.type](item.title)}`)
  })
